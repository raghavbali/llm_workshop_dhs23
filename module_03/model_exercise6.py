...
class Adapter(nn.Module):
    def __init__(self,config):
        super().__init__()
        self.down_proj = nn.Linear(config["n_embd"], config["adapter_size"], bias=config["bias"])
        self.nl = nn.ReLU()
        self.up_proj = nn.Linear(config["adapter_size"], config["n_embd"], bias=config["bias"])
        self.dropout = nn.Dropout(config["dropout"])
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, prompts=None):
        ...
        if hasattr(self,'c_head'):
            ...
            if targets is not None:
                ...
                loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=self.config.get("pad_token",-100))
            else:
    ...
    def extend_vocab(self, n_added_tokens, pad_token=None):
        ...
        with torch.no_grad():
            ...
            new_embeddings = nn.Embedding(, ,padding_idx=self.config.get("pad_token",None))
